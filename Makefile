...
ifeq ($(findstring --network zksync-sepolia,$(ARGS)),--network zksync-sepolia)
	NETWORK_ARGS := --rpc-url $(SEPOLIA_ZK_RPC_URL) --account sepolia --broadcast -vvvv
endif

ifeq ($(findstring --network zksync,$(ARGS)),--network zksync)
	NETWORK_ARGS := --rpc-url $(MAINNET_ZK_RPC_URL) --account sepolia --broadcast --verify --etherscan-api-key $(ETHERSCAN_API_KEY) -vvvv
endif
